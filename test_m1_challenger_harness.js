...
// -----------------------------------------------------------------------------
// 1. FILE SYNCHRONIZATION (SHA256) & SYNTAX CHECK
// -----------------------------------------------------------------------------
console.log('--- TEST 1: SHA256 File Synchronization & Node Syntax ---');
...
const rootGameHash = getSha256(path.join(__dirname, 'game.js'));
const assetsGameHash = getSha256(path.join(__dirname, 'assets', 'game.js'));
assert(rootGameHash === assetsGameHash, `game.js SHA256 match (${rootGameHash.substring(0, 10)}...)`);

const rootIndexHash = getSha256(path.join(__dirname, 'index.html'));
const assetsIndexHash = getSha256(path.join(__dirname, 'assets', 'index.html'));
assert(rootIndexHash === assetsIndexHash, `index.html SHA256 match (${rootIndexHash.substring(0, 10)}...)`);
...
const gameJsCode = fs.readFileSync(path.join(__dirname, 'game.js'), 'utf8');
...
try {
  ...
  console.log('✅ game.js evaluated successfully in mock environment');
  ...
  console.error('❌ Error evaluating game.js:', e);
  process.exit(1);
}
