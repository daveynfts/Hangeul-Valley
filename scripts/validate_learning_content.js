...
function assertEnglish(value, label) {
  ...
  assert(!VIETNAMESE_DIACRITICS.test(value), `${label} must not contain Vietnamese text`);
}
...
assert.strictEqual(curriculum.locale, 'en-US', 'Curriculum must target English-speaking learners');
...
curriculum.chapters.forEach((chapter, chapterIndex) => {
  ...
  assertEnglish(chapter.titleVi, `${chapter.id}.titleVi`);
  assertEnglish(chapter.canDoVi, `${chapter.id}.canDoVi`);
  ...
  chapter.grammar.forEach(point => {
    ...
    assertEnglish(point.meaningVi, `${chapter.id}.${point.id}.meaningVi`);
    ...
    point.examples.forEach(example => {
      ...
      assertEnglish(example.vi, `${point.id}.example.vi`);
    });
  });
  ...
  chapter.dialogue.forEach(line => {
    ...
    assert(line.speaker && line.vi, `${chapter.id} dialogue lines need speaker and English meaning`);
    assertEnglish(line.vi, `${chapter.id}.dialogue.vi`);
  });
  ...
  chapter.missions.forEach(mission => {
    ...
    assertEnglish(mission.promptVi, `${mission.id}.promptVi`);
    assertEnglish(mission.explanationVi, `${mission.id}.explanationVi`);
...
for (const file of [
  ...
  assert(!VIETNAMESE_DIACRITICS.test(read(file)), `${file} must not contain Vietnamese player-facing text`);
...
const worldScriptAt = html.indexOf('<script src="world-learning.js"></script>');
const gameScriptAt = html.indexOf('<script src="game.js"></script>');
const worldUiScriptAt = html.indexOf('<script src="world-learning-ui.js"></script>');
...
assert(learningScriptAt < worldScriptAt && worldScriptAt < gameScriptAt, 'world-learning.js must load before game.js');
assert(gameScriptAt < worldUiScriptAt, 'world-learning-ui.js must load after game.js');
...
assert(html.includes('id="world-learning-overlay"'), 'World Learning overlay is missing');
assert(html.includes('onclick="openWorldMissionMap()'), 'World mission map entry point is missing');
...
assert(game.includes('v: 8'), 'Save schema v8 is required for world learning progress');
assert(game.includes('visual: visualState'), 'Visual world state must be persisted with the save');
assert(game.includes('worldLearning: worldLearningState'), 'World learning evidence must be persisted with the save');
