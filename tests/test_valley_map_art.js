...
assert(manifest.entries.length === 9, 'manifest covers the nine redesigned map assets');
assert(manifest.entries.every((entry) => entry.reviewed), 'every source and processed sprite was reviewed');
assert(new Set(manifest.entries.map((entry) => entry.role)).size === 9, 'each map role has one dedicated asset');
...
assert(/Math\.max\(72, this\.farm\.x - 150\)/.test(farm), 'arcade stays inside a 576px map');
