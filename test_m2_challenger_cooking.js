...
// -----------------------------------------------------------------------------
// 1. SHA256 Byte-for-Byte Synchronization Check
// -----------------------------------------------------------------------------
console.log('\n--- SECTION 1: Byte-for-Byte SHA256 Synchronization ---');
...
const gameJsHash = getSha256('game.js');
const assetsGameJsHash = getSha256('assets/game.js');
assert(
  gameJsHash === assetsGameJsHash,
  'SHA256 Synchronization: game.js <-> assets/game.js',
  `game.js (${gameJsHash.substring(0, 12)}) vs assets/game.js (${assetsGameJsHash.substring(0, 12)})`
);

const indexHtmlHash = getSha256('index.html');
const assetsIndexHtmlHash = getSha256('assets/index.html');
assert(
  indexHtmlHash === assetsIndexHtmlHash,
  'SHA256 Synchronization: index.html <-> assets/index.html',
  `index.html (${indexHtmlHash.substring(0, 12)}) vs assets/index.html (${assetsIndexHtmlHash.substring(0, 12)})`
);
...
try {
  ...
  assert(true, 'VM Context Load: game.js loaded without syntax/runtime errors');
} catch (err) {
  assert(false, 'VM Context Load: game.js failed to execute', err.stack);
  process.exit(1);
}
